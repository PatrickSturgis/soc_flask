...
class SOCClassifier:
    ...
    def _parse_classify_response(self, response: str, soc_cands: str) -> Dict[str, str]:
        """Parse static classification response (force_classify format)."""
        if response.startswith("CGPT587:"):
            try:
                # Format: CGPT587: 2312 - Description (85)
                soc_code = re.findall(r"(?<=CGPT587:\s)\d{4}", response)[0]
                soc_desc = re.findall(r"(?<=CGPT587:\s\d{4}\s-\s).*(?=\s\(\d+\))", response)[0]
                soc_conf = re.findall(r"(?<=\()\d+(?=\))", response)[0]
                soc_followup = "N/A"  # Static mode - no follow-ups
